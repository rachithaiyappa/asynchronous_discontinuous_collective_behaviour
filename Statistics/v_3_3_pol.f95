...
!v_3_1 is aimed at seeing the polarisation of the group
...
!(Personal reference) Another change being the intial postions of the 
!fish is now made random
...
do i=2,8,1
	...
	write(*,*)"gamma_ att=",gamma_att(i)
end do
do i=2,6,1
	gamma_rand(i) = gamma_rand(i-1)+0.05
	write(*,*) "gamma_rand=",gamma_rand(i)
end do
!*********************************************************************
...
!sqrt(<r^2>/N)--------------------------------------------------------
write(cmd_1,'(a)') 'mkdir sqrt_r_sq_by_N'
write(*,*) cmd_1
call system(cmd_1,status=md_status)

!P2-------------------------------------------------------------------
write(cmd_1,'(a)') 'mkdir P2'
write(*,*) cmd
call system(cmd_1,status=md_status)

!fnn------------------------------------------------------------------
write(cmd_1,'(a)') 'mkdir fnn'
write(*,*) cmd
call system(cmd_1,status=md_status)
!---------------------------------------------------------------------
...
!writing radius of gyration into a file-------------------------------
summ = 0.0
do a=1,N,1
	do c=a+1,N,1
...
do while (time_limit > maxval(time(:,col)))
	...
	do a=1,N,1
		do c=a+1,N,1
			summ= summ+(((x_temp(a,col)-        &
 		x_temp(c,col))**2) +((y_temp(a,col)-	    &
				y_temp(c,col))**2))
		end do
	end do
	r_sq(col) = (2.0/(N*(N-1)))*summ
	...
	a = 1
	do while (time_mod(1,a) <= 1000)
		stat_start = a
		a = a + 1
	end do
	write(*,*) stat_start, col
	!storing dist_fnn--------------------------------------------
	call chdir('fnn')
	!opening the file to store fnn distance
	write(filename,'("fnn",i3.3,".txt")') i
	...
	write(*,*) "fnn_file_status",fnn_file_status 
	summ = 0
	do a=stat_start,col-1
		summ = summ + (sum(d_fnn(1:N,a))/N)
		!write(*,*) sum(d_fnn(1:N,a))/N
	end do
	write(11,*) gamma_rand(f),gamma_att(e),			&
		summ/((col-1)-(stat_start)+1)
	call chdir('..')
	!*************************************************************
	...
	write(12,*) gamma_rand(f),gamma_att(e),			&
	summ/((col-1)-(stat_start)+1)
	call chdir('..')
	!************************************************************

	!storing sqrt(<r^2>/N)---------------------------------------
	call chdir('sqrt_r_sq_by_N')
	!opening the file to store r^2(t)
	write(filename,'("r_sq_time",i3.3,".txt")') i
	open (unit=13, file=filename,status="replace",		&
		action="write",position="rewind",		&
	...
	write(13,*) gamma_rand(f),gamma_att(e),			&
sqrt(summ/(N*((col-1)-(stat_start)+1)))
	call chdir('..')
	!************************************************************
